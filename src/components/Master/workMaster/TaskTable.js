...
import { Tabs, Table, Dropdown, Button, Input, Menu, Space, Switch, Tooltip } from 'antd';
...
function EditData(props) {
    const { selectedRow,insertCalenderData, fetchData } = props
    return (
        <Modal
            {...props}
            size="xl"
            ...
            backdrop="static"
        >
            <TaskForm rowData={selectedRow} fetchData={fetchData} onHide={props.onHide} fetchCalenderData={insertCalenderData} />
        </Modal>
    );
}

function TaskTable({insertCalenderData, getTaxadmindata, getCategorydata, insertData }) {
    // console.log(insertCalenderData,'insertCalenderData')
    React.useEffect(() => {
    ...
    const actionTemplate = (rowData) => {
            ...
            //     <button type="button" className="btn btn-add action_btn btn-sm rounded-2" onClick={() => { showAlert(rowData) }}><i className="fa fa-trash fs-4" /></button>
            //     {/* <button type="button" className="btn btn-add action_btn btn-sm rounded-2" onClick={() => { updatedata(rowData) }}><i className="fa fa-pencil fs-4" /></button> */}
            ...
            <div className="action-btn">
                ...
                <Tooltip title="TimeLine" >
                    <button className="log-btn"
                        onClick={() => { handleLogDetails(rowData.TicketNo) }}
                    ><FaRegEye size={18} />
                    </button>
                </Tooltip>
    ...
    const assignToTemplate = (rowData) => {
        const assignToNames = rowData.ATFName + ' ' + rowData.ATLName // Assuming AssignTo is a comma-separated list of names
            ...
            >
                {projectNames.map((projectName) => (
                    <TabPane tab={projectName} key={projectName}>
                        ...
                        {selectedTab === 'all' && (
                            // <DataTable
                            //     value={data.filter((item) => item.ProjectName === projectName)}
                            //     paginator rows={10}
                            //     rowsPerPageOptions={[5, 10, 25, 50]}>
                            //     <Column
                            //         body={(rowData) => rowData.Prefix + rowData.TicketNo}
                            //         header="Ticket No"
                            //     />
                            ...
                            //     />
                            // </DataTable>
                            <Table dataSource={data.filter((item) => item.ProjectName === projectName)} columns={columns} size='small' scroll={{ x: 500 }} pagination={tableParams.pagination}
                                onChange={handleTableChange} />
                        )}
                        {selectedTab === 'runningtask' && (
                            <Table dataSource={data.filter((item) => item.ProjectName === projectName && item.TaskStatus === 'InProgress')} columns={columns} size='small' scroll={{ x: 500 }} pagination={tableParams.pagination}
                                onChange={handleTableChange} />
                        )}
                        {selectedTab === 'pendingtask' && (
                            <Table dataSource={data.filter((item) => item.ProjectName === projectName && item.TaskStatus === 'Pending')} columns={columns} size='small' scroll={{ x: 500 }} pagination={tableParams.pagination}
                                onChange={handleTableChange} />
                        )}
                        {selectedTab === 'completedtask' && (
                            <Table dataSource={data.filter((item) => item.ProjectName === projectName && item.TaskStatus === 'Complete')} columns={columns} size='small' scroll={{ x: 500 }} pagination={tableParams.pagination}
                                onChange={handleTableChange} />
                        )}
                        {selectedTab === 'holdtask' && (
                            <Table dataSource={data.filter((item) => item.ProjectName === projectName && item.TaskStatus === 'Hold')} columns={columns} size='small' scroll={{ x: 500 }} pagination={tableParams.pagination}
                                onChange={handleTableChange} />
                        )}
                        {selectedTab === 'canceltask' && (
                            <Table dataSource={data.filter((item) => item.ProjectName === projectName && item.TaskStatus === 'Cancel')} columns={columns} size='small' scroll={{ x: 500 }} pagination={tableParams.pagination}
                                onChange={handleTableChange} />
                        )}
