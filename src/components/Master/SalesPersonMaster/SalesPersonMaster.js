...
import PartyTable from './SalesPersonTable';
...
import PartyRecordImport from './SalesPersonRecordImport'
...
function PartyNewForm(props) {
    const { fetchData, getSalesPersonData } = props;
    return (
        <Modal
            {...props}
            size="xl"
            ...
            backdrop="static"
        >
            <PartyForm getSalesPersonData={getSalesPersonData} fetchData={fetchData} onHide={props.onHide} />
        </Modal>
    );
}

const PartyMaster = ({ getPartyData, onHide, getSalesPersonData }) => {
    ...
    const generatePDF = () => {
        ...
        doc.text('Sales Person List :-', leftMargin, 19);
        ...
        doc.autoTable({
            head: [['No', 'Sales Person Name', 'Legal Name', "Address", 'State', 'Mobile', 'Date of Birth', 'Date Of Join', 'Date of Anniversary', 'PAN', 'GST']],
            body: tableData,
            startY: 20,
        });

        doc.save('SalesPerson.pdf');
    };
    // const downloadExcel = () => {
    //     const worksheet = XLSX.utils.json_to_sheet(data);
    //     const workbook = XLSX.utils.book_new();
    //     XLSX.utils.book_append_sheet(workbook, worksheet, 'Sheet1');
    //     XLSX.writeFile(workbook, 'SalesPerson.xlsx');
    // };
    const downloadExcel = () => {
        const columeName = ['No', 'Sales Person Name', 'Legal Name', "Address", 'State', 'Mobile', 'Date of Birth', 'Date Of Join', 'Date of Anniversary', 'PAN', 'GST'];
        const formattedData = [
            columeName,
            ...data.map((item, index) => [
                index + 1,
              ...
              <table>
                ...
                <tbody>
                  ${data.map((item, index) => `
                    <tr>
                    <td>${index + 1}</td>
                      <td>${item.SalesPersonName ? item.SalesPersonName : '-'}</td>
    ...
    return (
        <div className={getSalesPersonData ? '' : 'content-wrapper'}>
                ...
                <div className="row">
                    <div className="col-lg-12 pinpin">
                        <div className="card lobicard" data-sortable="true">
                            <div className="card-header">
                                <div className='title-download-section'>
                                    <div className="card-title custom_title">
                                        <h4 className='report-heading'>Sales Person List</h4>
                                    </div>
                            ...
                            <div className="btn-group d-flex input-searching-main pt-3 pl-3 ps-3" role="group">
                                <div className="buttonexport" id="buttonlist">
                                    <Button className="btn btn-add rounded-2" onClick={() => setPartyNew(true)}>
                                        <i className="fa fa-plus" /> Add Sales Person [F2]
                                    </Button>
                                    <PartyNewForm
                                        show={partynew}
                            ...
                            <div className='p-3' >
                                <PartyTable insertData={insertData} searchinput={searchinput} ondata={handleData} getSalesPersonData={getSalesPersonData} importdata={importdata} />
                            </div>
                        </div>
                    </div>
                </div>
