...
const ProformaData = ({ onDataChange, onDataUpdate, selectedporformadata, ondeleteRow, proformagetdatasales, selectproforma, partyName }) => {
  ...
  useEffect(() => {
    if (proformagetdatasales) {
      const SalesData = proformagetdatasales.map(obj => {
  ...
  }, [proformagetdatasales])

  useEffect(() => {
    let performitemRecord;
    if (selectedporformadata) {
      performitemRecord = selectedporformadata.map(detail => ({
  ...
  const handleDeleteRow = async (porfirmacGuid, rowIndex) => {
    const deleteData = [...data];
    if (porfirmacGuid) {
      try {
        const res = await axios.get(URL + `/api/Transation/DeleteTransation?TransMode=${location.pathname == '/proformaentry' && "Poforma" || location.pathname == '/invoiceentry' && 'Invoice'}&CGUID=${porfirmacGuid}`, {
  ...
  //   // Extract the selected item's ID
  //   const selectedItemId = selectedItem || data[rowIndex].ItemId;

  //   // Update a variable (itemId) outside the state for potential use elsewhere
  //   itemId = selectedItemId;

  //   // Update the state for the selected option
  //   const updatedData = data.map((row, index) => {
  //     if (index === rowIndex) {
  //       // If the current row matches the selected row index, update the ItemId property
  //       return { ...row, ItemId: selectedItemId };
  //     }
  //     return row;
  //   });
  ...
  //   // Update the state with the selected item's ID
  //   setItemId(selectedItemId);
  // };
