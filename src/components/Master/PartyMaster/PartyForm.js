...
import { setFileNoValue } from "../common/common";
const { TabPane } = Tabs;
...
const validationSchema = Yup.object().shape({
  ...
  itFileNo: Yup.string()
    ...
    .test({
      ...
      test: function (value) {
        // Check if the lowercase value matches any of the lowercase forbidden strings
        const forbiddenStrings = ["CON", "PRN", "AUX", "NUL", "COM0", "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9", "LPT0", "LPT1", "LPT2", "LPT3", "LPT4", "LPT5", "LPT6", "LPT7", "LPT8", "LPT9"];
        return !forbiddenStrings.includes(value?.toUpperCase());
      },
      message: "Invalid IT File No.",
    }),
  gstFileNo: Yup.string()
    .nullable() // Allow null or empty value
    .test({
      name: "forbiddenStrings",
      test: function (value) {
...
const PartyForm = ({ getPartyData, onHide, rowData, fetchData }) => {
  // const PartyForm = ({ getPartyData, onHide, rowData, fetchData, errorData, reset_Data }) => {
  ...
  useEffect(() => {
    if (rowData) {
      ...
      setItFileNo(rowData.ITFileNo);
      setGSTFileNo(rowData.GSTFileNo);
      setOtherFileNo(rowData.OtherFileNo);
      setCINNo(rowData.CompanyCINNo);
      setCompanyStatus(rowData.CompanyStatus);
      setResidentStatus(rowData.ResidentStatus);
      setGSTFillingStatus(rowData.GSTFillingStatus);
      setGSTID(rowData.GSTId);
      setGSTPassword(rowData.GSTPassword);
      setIncomeTaxID(rowData.IncomTaxId);
      setIncomeTaxPassword(rowData.IncomTaxPassword);
      setUserID(rowData.TraceId);
      setUserPassword(rowData.TracePassword);
      setPartnerName(rowData.PartnerId);
      setSelectedEmp(rowData.EmpId)
      setEmail1(rowData.Email1)
  ...
  const fetchProjectData = async () => {
    try {
      const res = await axios.get(
        URL + `/api/Master/ProjectList?CompanyId=${compnayid}`,
        {
          headers: { Authorization: `bearer ${token}` },
        }
      );
  ...
  const getPartnerList = async () => {
    try {
      const res = await axios.get(
        URL + `/api/Master/PartnerList?CustId=${custId}&CompanyId=${compnayid}`,
        {
          headers: { Authorization: `bearer ${token}` },
        }
      );
      setPartnerDataList(res.data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const getEmployeeList = async () => {
    try {
      const res = await axios.get(URL + `/api/Master/GetEmpList?CustId=${custId}&CompanyId=${compnayid}`, {
        headers: { Authorization: `bearer ${token}` }
      })
      setEmpList(res.data)
    } catch (error) {
  ...
  const getEmployeOption = empList?.map((display) => ({
    value: display.Id,
  ...
  const resident_Status = masterData.filter(
    (display) => display.Remark === "ResidentStatus"
  );
  const gstFilling_Status = masterData.filter(
    (display) => display.Remark === "GSTSTATUS"
  );
  const companyStatusOptions = company_Status.map((display) => ({
    value: display.MasterTag1,
    label: display.Description,
  }));
  const residentStatusOptions = resident_Status.map((display) => ({
    value: display.MasterTag1,
    label: display.Description,
  }));
  const gstFillingStatusOptions = gstFilling_Status.map((display) => ({
    value: display.MasterTag1,
    label: display.Description,
  }));
  ...
  const projectoption = projectData.map((display) => ({
    value: display.Id,
    label: display.ProjectName,
  }));
  const partneroption = partnerDataList.map((display) => ({
    value: display.PartnerId,
    label: display.PartnerName,
  }));
  // console.log(categoryoption, "categoryoption")

  const handleSelect = (selected) => {
    setProjectName(selected.map((option) => option.value));
    const project = selected.map((option) => option.value).join(",");
  ...
  const toggleGSTPasswordVisibility = () => {
    setShowGSTPassword(!showGSTPassword);
  };
  const toggleUserITVisibility = () => {
    setShowITPassword(!showITPassword);
  };
      ...
      if (partyid >= 0) {
        const res = await axios.post(
          URL + "/api/Master/CreateParty",
          {
            ...
            DOJ: doj,
            Add1: add1 && addressFieldChange(add1.trim()),
            Add2: add2 && addressFieldChange(add2.trim()),
            Add3: add3 && addressFieldChange(add3.trim()),          
                ...
                >
                  <div className="col-sm-12">
                    <Tabs defaultActiveKey="1" transition={true}>
                      ...
                      >
                          ...
                          <Col lg={6}>
                            <div className="form-group">
                              ...
                              <Select
                                ...
                                // )}
                                value={projectoption?.filter((option) =>
                                  projectName?.includes(option.value)
                                )}
                        ...
                        <Row>
                          <Col lg={4}>
                            <div className="form-group">
                              ...
                              <Select
                                ...
                                // isMulti
                                isClearable={true}
                                value={partneroption.filter(
                                  (option) => option.value == partnerName
                                )}
                          ...
                          <Col lg={4}>
                            <div className="form-group">
                              ...
                              <Select
                                options={getEmployeOption}
                                isClearable={true}
                                value={getEmployeOption?.find(
                                  (option) => option.value == selectedemp
                                )}
                        ...
                        <Row>
                          <Col lg={6}>
                            <div className="form-group">
                              <label>
                                Party Name / Company Name / Trade name :
                              ...
                              <input
                                ...
                                placeholder="Enter Party Name / Company Name"
                              />
                              {errors.partyname && (
                      ...
                      <TabPane key="2" tab="IT / GST Details">
                        <Row>
                          <Col lg={6}>
                            <div className="form-group">
                              ...
                              <input
                                ...
                                onChange={(event) => {
                                  ...
                                  const inputValue = input.value
                                    // .toUpperCase()
                                    // .replace(/[^A-Z0-9]/g, "")
                                    .slice(0, 10); // Allow only capital letters and numbers;
                                  input.value = inputValue;
                                  input.setSelectionRange(start, end);
                                  setItFileNo(setFileNoValue(inputValue));
                                  if (errors.itFileNo) {
                                    setErrors((prevErrors) => ({
                                      ...prevErrors,
                                      itFileNo: null,
                            ...
                            <div className="form-group">
                              ...
                              <input
                                ...
                                onChange={(event) => {
                                  ...
                                  input.value = inputValue;
                                  input.setSelectionRange(start, end);
                                  setGSTFileNo(setFileNoValue(inputValue));
                                  if (errors.gstFileNo) {
                                    setErrors((prevErrors) => ({
                                      ...prevErrors,
                                      gstFileNo: null,
                            ...
                            <div className="form-group">
                              ...
                              <input
                                ...
                                onChange={(event) => {
                                  ...
                                  input.value = inputValue;
                                  input.setSelectionRange(start, end);
                                  setOtherFileNo(setFileNoValue(inputValue));
                                  if (errors.otherFileNo) {
                                    setErrors((prevErrors) => ({
                                      ...prevErrors,
                                      otherFileNo: null,
                                    }));
                            ...
                            <div className="form-group">
                              ...
                              <input
                                ...
                                onChange={(event) => {
                                  ...
                                  //   .toUpperCase()
                                  //   .replace(/[^A-Z0-9]/g, "")
                                  //   .slice(0, 25); // Allow only capital letters and numbers;
                                  // input.value = inputValue;
                                  // input.setSelectionRange(start, end);
                                  setCINNo(input);
                                }}
                          ...
                          <Col lg={6}>
                            <div className="form-group">
                              ...
                              <Select
                                ...
                                // isMulti
                                isClearable={true}
                                value={residentStatusOptions.find(
                        ...
                        <Row>
                          <Col lg={6}>
                            <div className="form-group">
                              ...
                              <Select
                                options={gstFillingStatusOptions}
                                // isMulti
                                isClearable={true}
                                value={gstFillingStatusOptions.find(
                                  (option) => option.value == gstFillingStatus
                                )}
                                placeholder="Select Filling Mode"
                                onChange={(select) => {
                                  setGSTFillingStatus(
                                    select ? select.value : ""
                                  );
                      ...
                      <TabPane key="3" tab="Address Details">
                        <Row>
                          <Col lg={12}>
                            <div className="form-group">
                      ...
                      <TabPane key="4" tab="Bank Details">
                        <Row>
                          <Col lg={6}>
                            <div className="form-group">
                      ...
                      <TabPane key="5" tab="Login Details">
                        <Row>
                          <Col lg={6}>
                            <div className="form-group">
                              <label>Deductor User ID (TRACES ):</label>
                              <input
                                type="text"
                                className="form-control"
                                placeholder="Enter Deductor User ID"
                                value={userID}
                                onChange={(event) => {
                                  const input = event.target;
                                  const start = input.selectionStart;
                          ...
                          <Col lg={6}>
                            <div className="form-group">
                              <label>Deductor User Password (TRACES ):</label>
                              <div className="password-main-show-hide">
                                <input
                                  type={showUserPassword ? "text" : "password"}
                                  className="form-control"
                                  placeholder="Enter Deductor User Password"
                                  value={userPassword}
                                  onChange={(event) => {
                                    const input = event.target;
                                    const start = input.selectionStart;
                            ...
                            <div className="form-group">
                              ...
                              <div className="password-main-show-hide">
                                ...
                                <button
                                  className="psw-show-icon"
                                  type="button"
                                  onClick={toggleGSTPasswordVisibility}
                                >
                                  {showGSTPassword ? (
                                    <AiOutlineEyeInvisible />
                                  ) : (
                            ...
                            <div className="form-group">
                              ...
                              <div className="password-main-show-hide">
                                ...
                                <button
                                  className="psw-show-icon"
                                  type="button"
                                  onClick={toggleUserITVisibility}
                                >
                                  {showITPassword ? (
                                    <AiOutlineEyeInvisible />
                                  ) : (
