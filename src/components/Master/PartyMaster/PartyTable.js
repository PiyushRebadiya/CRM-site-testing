...
function EditData(props) {
    const { selectedrow, fetchData, getPartyData } = props
    return (
        <Modal
            {...props}
            size="xl"
            ...
            backdrop="static"
        >
            <PartyForm rowData={selectedrow} fetchData={fetchData} onHide={props.onHide} getPartyData={getPartyData} />
        </Modal>
    );
}
// function EditData(props) {
...
function NewData(props) {
    ...
    return (
        <Modal
            ...
            backdrop="static"
        >
            <DocumentForm onHide={props.onHide} rowData={selectedrow} partyID={partyID} />
        </Modal>
    );
}
const PartyTable = ({ getPartyData, insertData, searchinput, datarecord, ondata, importdata }) => {
    React.useEffect(() => {
    ...
    const filteredData = data.filter((item) => {
        ...
        const birthDay = moment(item.DOB).format('DD/MM/YYYY').toLowerCase();
        const dateOfJoining = moment(item.DOJ).format('DD/MM/YYYY').toLowerCase();
        const dateOfAnniversary = moment(item.AnnivarsaryDate).format('DD/MM/YYYY').toLowerCase();
        return (
            // item.PartyName.toLowerCase().includes(searchTermLowerCase)
            ...
            (item.GST && item.GST.toLowerCase().includes(searchTermLowerCase)) ||
            (birthDay && birthDay.includes(searchTermLowerCase)) ||
            (dateOfJoining && dateOfJoining.includes(searchTermLowerCase)) ||
            (dateOfAnniversary && dateOfAnniversary.includes(searchTermLowerCase)) 
        );
    });
    ...
    const deleteData = async (id, timerDuration) => {
        try {
            const res = await axios.get(URL + `/api/Master/DeleteParty?PartyId=${id}`, {
                headers: { Authorization: `bearer ${token}` },
            })
