...
import PartyImportRecord from './AgentImportRecord';
...
function EditData(props) {
    const { selectedrow, fetchData, getAgentData } = props
    return (
        <Modal
            {...props}
            size="xl"
            ...
            backdrop="static"
        >
            <PartyForm rowData={selectedrow} fetchData={fetchData} onHide={props.onHide} getAgentData={getAgentData} />
        </Modal>
    );
}
// function EditData(props) {
...
function NewData(props) {
    ...
    return (
        <Modal
            ...
            backdrop="static"
        >
            <DocumentForm title={"Agent"} onHide={props.onHide} rowData={selectedrow} partyID={partyID} />
        </Modal>
    );
}
const PartyTable = ({ getAgentData, insertData, searchinput, datarecord, ondata, importdata }) => {
    React.useEffect(() => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await axios.get(URL + `/api/Master/AgentList?CustId=${customerId}&CompanyId=${companyid}`, {
    ...
    const filteredData = data.filter((item) => {
        ...
        return (
            ...
            (item.AgentName && item.AgentName.toLowerCase().includes(searchTermLowerCase)) ||// Check for null
    ...
    const updateData = async (rowData) => {
        try {
            const res = await axios.get(URL + `/api/Master/AgentListById?AgentId=${rowData.AgentId}`, {
    ...
    const deleteData = async (id, timerDuration) => {
        try {
            const res = await axios.get(URL + `/api/Master/DeleteAgent?AgentId=${id}`, {
