...
import InquiryReportTable from './InquiryReportTable';
// import TaskReportAssignByMeTable from './TaskReportAssignByMe';
import InquiryAssignByMe from './InquiryAssignByMe';
...
const TaskReportList = async () => {
    try {
        const res = await axios.get(URL + 
            `/api/Master/GetTaskReport?PartyId=0&TaskStatus=&startdate=${formattedfrom}&endDate=${formattedto}&CompanyId=${CompanyId}&Type=Deal&AssignTo=${role == 'Admin' ? '' : Userid}&ProjectId=0&CategoryId=0&TaxadminId=0`, {
...
const TaskReportAssignByMeList = async () => {
    try {
        const res = await axios.get(URL + `/api/Master/GetTaskReport?PartyId=0&TaskStatus=&startdate=${formattedfrom}&endDate=${formattedto}&CompanyId=${CompanyId}&Type=Deal&AssignTo=&AssignBy=${role == 'Admin' ? '' : Userid}&ProjectId=0&CategoryId=0&TaxadminId=0`, {
...
const DataSubmit = async () => {
    ...
    if(role =='Admin'){
        try {
            const res = await axios.get(URL + `/api/Master/GetTaskReport?PartyId=${selectedparty}&TaskStatus=${selectedTask}&startdate=${fromdate}&endDate=${todate}&CompanyId=${CompanyId}&Type=Deal&AssignTo=${role == 'Admin' ? selectedemp : Userid}&AssignBy=&ProjectId=${projectname}&CategoryId=${category}&TaxadminId=${taxadmin}`,
             {
                headers: { Authorization: `bearer ${token}` }
            })
            setData(res.data)
        ...
        try {
            const res = await axios.get(URL + `/api/Master/GetTaskReport?PartyId=${selectedparty}&TaskStatus=${selectedTask}&startdate=${fromdate}&endDate=${todate}&CompanyId=${CompanyId}&Type=Deal&AssignTo=&AssignBy=${role == 'Admin' ? '' : Userid}&ProjectId=${projectname}&CategoryId=${category}&TaxadminId=${taxadmin}`, {
...
const DataAssignByMeSubmit = async () => {
    setLoading(true);
    try {
        const res = await axios.get(URL + `/api/Master/GetTaskReport?PartyId=0&TaskStatus=${selectedAssignByMeTask}&startdate=${fromASBdate}&endDate=${ToASBdate}&CompanyId=${CompanyId}&Type=Deal&AssignTo=&AssignBy=${role == 'Admin' ? '' : Userid}&ProjectId=0&CategoryId=0&TaxadminId=0`, {
    ...
    const generatePDF = () => {
        ...
        // const leftMargin = 15;
        doc.text('Inquiry Report List :-', 15, 44);
        ...
        doc.autoTable({
            head: [['No', 'Party Name', 'Project Name', 'CategoryName', 'Inquiry Name', 'Start Date', 'End Date', 'AssignBy', 'AssignTo', 'Inquiry Status']],
            body: tableData,
            startY: 48,
        });
    ...
    const downloadExcel = () => {
        ...
        XLSX.writeFile(workbook, 'Inquiry Report.xlsx');
    };
                ...
                <div className='content-wrapper'>
                    {/* <section className="content-header">
                        ...
                        <div className="header-title">
                            <h1>{`${location.pathname == '/inquiryregister' && 'Inquiry'} Report`}</h1>
                        </div>
                    </section> */}
                      <section className="content-header">
                        <div className="header-icon">
                                            ...
                                            <div className='filter-label date-section-main p-1 mt-4'>
                                                ...
                                                <div className='w-100'>
                                                    <Select
                                                        ...
                                                        onChange={(selected) => {
                                                            setCategory(selected ? selected.value : 0)
                                                            // if (errors.party) {
                                                            //     setErrors(prevErrors => ({ ...prevErrors, party: '' }));
                                                            // }
                                                        }}
                                                        ...
                                                        <div className='report-date-main-section'>
                                                            <Row>
                                                                ...
                                                                <Col xl={2} >
                                                                    <div className='ml-4 submit-record-data'>
                                                                        <div className='report-submit-btn'>
                                                                            <button onClick={DataSubmitUser} disabled={loading}>
                                                                                {loading ? 'Submit...' : 'Submit [F9]'}
                                                                            </button>
                                                                        </div>
                                                                        <div className='report-submit-btn p-1'>
                                    ...
                                    <section className="px-4 assignbyreport">
                                        ...
                                        <div className="buttonexport" id="buttonlist">
                                                    ...
                                                    <div className='p-3' >
                                                        ...
                                                        <InquiryAssignByMe data={assignByMedata} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
export default InquiryReportMaster