...
function EditData(props) {
    ...
    return (
        <Modal
            ...
            backdrop="static"
        >
            <SalesSetting rowData={selectedRow} fetchData={fetchData} onHide={props.onHide} />
        </Modal>
    );
}

const SalesSettingTable = ({ insertData, searchinput }) => {
    React.useEffect(() => {
    ...
    const fetchData = async () => {
        try {
            const res = await axios.get(URL + `/api/Master/SettingList?CompanyId=${companyId}`, {
...
export default SalesSettingTable