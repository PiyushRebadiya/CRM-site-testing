...
const SalesTable = ({ onDataChange, onDataUpdate, selectedsalesdata, ondeleteRow, salesgetdatasales, selectsales, partyName }) => {

  // console.log(proformagetdatasales, "proformagetdatasales---------")
  // console.log(selectproforma, "selectproforma")
  ...
  useEffect(() => {
    if (salesgetdatasales) {
      const SalesData = salesgetdatasales.map(obj => {
        const { TransID, CGUID, ...rest } = obj;
        return rest;
      });
      const salesproformadata = SalesData.map(detail => ({
        // CGUID:detail.CGUID,
  ...
  useEffect(() => {
    ...
    if (selectedsalesdata) {
      performitemRecord = selectedsalesdata.map(detail => ({
        // CGUID:detail.CGUID,
        TransID: detail.TransID,
        ItemId: detail.ItemId,
        Description: detail.Description,
  ...
  const handleDeleteRow = async (salescGuid, rowIndex) => {
    const deleteData = [...data];
    if (salescGuid) {
      try {
        const res = await axios.get(URL + `/api/Transation/DeleteTransation?TransMode=${location.pathname == '/Sales' && "Sales" || location.pathname == '/invoiceentry' && 'Invoice'}&CGUID=${salescGuid}`, {
  ...
  // useEffect(() => {
  //   const updatedData = [...data];
  //   let isValidTaxRate = true; // Flag to track if all tax rates are valid
  ...
  //     // Validate tax rate
  //     if (![0, 5, 12, 18, 28].includes(TAX)) {
  //       isValidTaxRate = false;
  //       // You can set an error message or handle the error in your preferred way
  //       // For example, you can log an error, display a notification, etc.
  //       // console.error(`Invalid TaxRate at row ${rowIndex + 1}: ${TAX}%
  //       // valid taxRate is {0,5,12,18,28}`);
  //       notification.error({
  //         message: `Invalid TaxRate at row ${rowIndex + 1}: ${TAX}%
  //         valid taxRate is {0%,5%,12%,18%,28%}`,
  ...
  //   if (isValidTaxRate && !deepEqual(updatedData, data)) {
  //     setData(updatedData);
  //   }
